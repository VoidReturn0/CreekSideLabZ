...
# Initialize Communications class for CAMERA
comm = Communications('CAMERA')
...
class VideoStream:
    ...
    def __init__(self, resolution=(640, 480), framerate=30):
        self.comm = Communications._instance  # Use the singleton instance
        self.comm.register_callback('CAMERA', self.handle_message)
...
while True:
    ...
    logging.debug("cam_Main loop iteration started")
    ...
    if valid_detections:
        # Sort by confidence score and pick the highest
        valid_detections.sort(key=lambda x: x[2], reverse=True)
        x_offset, y_offset, _, xmin, ymin, xmax, ymax = valid_detections[0]


        # Draw bounding box around the closest target
        cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), (0, 255, 0), 2)


        # Send the offsets of the closest target
        try:
            if comm.connection_event.is_set():
                message = f"x_offset:{x_offset},y_offset:{y_offset}"
